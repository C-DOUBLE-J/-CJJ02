...
import com.workers.utils.FileNameUtil;
...
public class ProductInfoAction {
    ...
    public Object ajaxImg(MultipartFile pimage, HttpServletRequest request){
        //提取生成的文件名UUID+上传图片的后缀
        saveFileName = FileNameUtil.getUUIDFileName()+FileNameUtil.getFileType(pimage.getOriginalFilename());
        //得到项目中图片存储的路径
        String path = request.getServletContext().getRealPath("/image_big");
        //转存 E:\JAVA-SSM框架\xiaomi-ssm\image_big\(File.separator就是个反斜杠)生成的UUID的随机值.jpg
        try {
            pimage.transferTo(new File(path+File.separator+saveFileName));
        } catch (IOException e) {
            e.printStackTrace();
        }
        //为了在客户端显示图片，要将存储的文件名回传下去，
        // 由于是自定义的上传插件，所以此处要手工处理JSON
        JSONObject object = new JSONObject();
        object.put("imgurl",saveFileName);
        //JSON对象一定要toString()回到客户端
    ...
    public String one(int pid, ProductInfoVo vo,Model model,HttpSession session){
        ...
        //将多条件和页码放入session中,更新处理结束后 分页时读取条件和页码进行处理
    ...
    public String delete(int pid,ProductInfoVo vo,HttpSession session){
        int num = -1;
        try {
    ...
    public Object deleteAjaxSplit(HttpSession session){
        PageInfo info = null;
        Object vo = session.getAttribute("deleteProdVo");
        if (vo !=null){
            info = productInfoService.splitPageVo((ProductInfoVo) vo,PAGE_SIZE);
            session.removeAttribute("deleteProVo");
        }else{
            info = productInfoService.splitPage(1,PAGE_SIZE);
        }
